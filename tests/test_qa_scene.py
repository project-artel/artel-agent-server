...
# --- rect history and the live view -------------------------------------------


def goblin(x: int, y: int, name: str = "Goblin") -> Visual:
    return Visual(id=44, name=name, type="sprite", rect=Rect(x=x, y=y, w=96, h=96))


def test_a_visuals_rect_is_tracked_across_frames() -> None:
    """The rect is replaced with the frame; its path is not. A sprite that moved
    while the agent was not looking is exactly what a step's `expected` is about."""
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(state(visuals=[goblin(100, 0)]))

    track = memory.visual_rects["Goblin"]
    # Deduped like an observable: the repeated frame recorded nothing.
    assert [point.value.x for point in track.values] == [0, 100]
    assert [point.at for point in track.values] == [1, 3]


def test_rect_history_is_bounded_like_an_observable() -> None:
    memory = SceneMemory()
    for x in range(MAX_VALUES_PER_OBSERVABLE + 5):
        memory.apply(state(visuals=[goblin(x, 0)]))
...
def test_visuals_sharing_a_name_get_their_own_track() -> None:
    """Keyed by name, so duplicates must be numbered — otherwise five `Enemy`
    sprites would splice their positions into one meaningless path."""
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0), goblin(500, 0)]))
    memory.apply(state(visuals=[goblin(10, 0), goblin(500, 0)]))

    assert [p.value.x for p in memory.visual_rects["Goblin"].values] == [0, 10]
    assert [p.value.x for p in memory.visual_rects["Goblin#2"].values] == [500]


def test_a_visual_that_leaves_the_scene_expires() -> None:
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    for _ in range(MISSING_LIFETIME + 1):
        memory.apply(state(visuals=[]))

    assert "Goblin" not in memory.visual_rects
    assert "Goblin" not in memory.visual_last_seen


def test_scene_change_drops_rect_history() -> None:
    """Same reason the observables go: a new scene is a new key space."""
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(state(scene="Battle", visuals=[goblin(0, 0)]))

    assert [p.at for p in memory.visual_rects["Goblin"].values] == [1]


def test_render_now_is_none_before_the_first_frame() -> None:
    assert SceneMemory().render_now() is None


def test_render_now_shows_the_whole_value_path_with_its_observations() -> None:
    """What a watermark diff cannot give once the watermark has moved past it."""
    memory = SceneMemory()
    memory.apply(state(observables={"Hp": {"value": 100}}))
    memory.apply(state(observables={"Hp": {"value": 80}}))
    memory.apply(state(observables={"Hp": {"value": 60}}))

    view = memory.render_now()

    assert "Hp: 100 → 80 → 60   [obs 1, 2, 3]" in view
    assert view.startswith(CURRENT_SCENE_START)
    assert view.endswith(CURRENT_SCENE_END)
    # Never the tool-result view's markers: nothing that folds one may touch it.
    assert SCENE_VIEW_START_PREFIX not in view


def test_render_now_omits_the_observation_list_for_a_value_that_never_changed() -> None:
    memory = SceneMemory()
    memory.apply(state(observables={"Hp": {"value": 100}}))

    assert "Hp: 100\n" in memory.render_now()


def test_render_now_reports_a_moved_sprite_by_its_centre() -> None:
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(state(visuals=[goblin(100, 0)]))
    ...
    assert "[44] Goblin (sprite) @ 148,48 96x96" in view
    # The size is printed once on the element line; repeating it on every entry
    # of an unchanged-size path is noise.
    assert "moved: 48,48 → 148,48   [obs 1, 2]" in view


def test_render_now_keeps_the_size_when_it_changed_too() -> None:
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(
        state(visuals=[Visual(id=44, name="Goblin", type="sprite", rect=Rect(x=0, y=0, w=48, h=48))])
    )

    assert "moved: 48,48 96x96 → 24,24 48x48" in memory.render_now()


def test_render_now_says_nothing_about_a_sprite_that_stayed_put() -> None:
    memory = SceneMemory()
    memory.apply(state(visuals=[goblin(0, 0)]))
    memory.apply(state(visuals=[goblin(0, 0)]))

    assert "moved:" not in memory.render_now()


def test_render_now_marks_a_value_that_left_the_scene() -> None:
    """It is still evidence — a dialog that closed is often the whole step."""
    memory = SceneMemory()
    memory.apply(state(observables={"Dialog.open": {"value": True}}))
    memory.apply(state(observables={}))

    assert "(gone from the scene)" in memory.render_now()


def test_render_now_carries_the_newest_actions_with_their_observations() -> None:
    """An action the game ran on its own is reported nowhere else: no tool
    dispatched it, so no ACTION_RESULT answers for it."""
    memory = SceneMemory()
    memory.apply(state(actions=[ActionRecord(target="Enemy", name="Attack", success=True, at="t1")]))
    memory.apply(
        state(actions=[ActionRecord(target="Door", name="Open", success=False, error="locked", at="t2")])
    )

    view = memory.render_now()

    assert "Enemy.Attack → ok   [obs 1]" in view
    assert "Door.Open → FAILED (locked)   [obs 2]" in view


def test_render_now_keeps_only_the_newest_actions() -> None:
    """Rewritten on every model call, unlike the tool view — forty lines a turn
    would cost more than the rest of the view."""
    memory = SceneMemory()
    memory.apply(
        state(
            actions=[
                ActionRecord(target="Enemy", name=f"Act{i}", success=True, at=f"t{i}")
                for i in range(MAX_ACTIONS_IN_LIVE_VIEW + 5)
            ]
        )
    )
    ...
    assert "Act0" not in view
    assert f"Act{MAX_ACTIONS_IN_LIVE_VIEW + 4}" in view
    assert view.count(" → ok") == MAX_ACTIONS_IN_LIVE_VIEW
