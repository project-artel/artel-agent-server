...
def _body(title: str) -> QaScenarioBody:
    return QaScenarioBody(title=title, description="d")
...
def test_run_executes_scenarios_in_order_and_resets_between() -> None:
    async def run() -> None:
        ...
        scenarios = [
            QaScenario(qa_try_id=101, test_scenario_id=1, scenario=_body("S1")),
            QaScenario(qa_try_id=102, test_scenario_id=2, scenario=_body("S2")),
            QaScenario(qa_try_id=103, test_scenario_id=3, scenario=_body("S3")),
        ]
        session_id, _ = await service.open(qa_run_id=9, game_instance_id=1, scenarios=scenarios)
