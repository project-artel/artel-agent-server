...
모델 `anthropic/claude-sonnet-5`, 온도 0.2(`app/llm/chat_model.py` 고정값), seed 없음.
arm (a) 3 run, (b)·(c) 각 2 run. 원본 응답은 `evals/scene_chain/output/`(gitignore)에 있고
프롬프트 전문·입력 sha256 이 각 파일에 실려 있다. 아래 수치는 전부 **실측**이다.

입력 sha256 — content_map `aa6dad9813115f33…`, 캡처 `141f6a91ed92278c…`, `wv2cs.py` `764ca5069e563789…`.

| | 정답률 (평균, 범위) | 맵 밖 골든 | 지어냄(pooled) | 과소연결 (7 중) | run 당 비용 | run 당 시간 |
|---|---|---|---|---|---|---|
| **조인 귀무가설** | **0.80** | 0/2 | 0% | 0 | $0 | 0s |
| (a) content_map | 0.70 (0.6–0.8) | **0/2** | **0%** | 2.0 (1–3) | $0.13 | 116s |
| (b) 의사 C# | 0.55 (0.5–0.6) | **2/2** (두 run 다) | 23.5% | 6.5 (6–7) | $0.44 | 216s |
| (c) 둘 다 | 0.75 (0.7–0.8) | 1/2 (0–2) | 45.8% | 2.5 (2–3) | $0.41 | 227s |

### 읽는 법 네 가지

1. **어느 arm 도 공짜 기계 조인을 못 이겼다.** 귀무가설 8/10 을 넘은 것은 개별 run 두 개
   (a 한 번, c 한 번)가 8/10 로 **동점**을 낸 것뿐이다. 평균은 셋 다 8/10 아래다.
2. **함정 셋(SC-8·SC-9·SC-10)은 7 run 중 한 번도 나오지 않았다.** 이슈가 예측한 세 가지
   지어냄 — 이름 충돌, 게임 상식 메우기, 쓰는 주체 오인 — 은 이 모델·이 프롬프트에서
   관측되지 않았다. **위험이 없다는 뜻이 아니라, 예측한 모양으로는 오지 않았다는 뜻이다.**
3. **맵 밖 사실은 의사 C# 을 준 arm 만 주웠다.** (b) 는 두 run 다 SC-6·SC-7 을 냈고,
   (a) 는 구조적으로 0/2 다. 이것은 표본 문제가 아니라 입력에 그 사실이 없어서다.
   **content_map 이 잃어버린 것이 있다는 증거이지, 에이전트가 못 읽는다는 증거가 아니다.**
4. **의사 C# 은 질문에서 멀어진다.** (b) 는 33~35개 체인을 내면서 카드·적·튜토리얼 시스템으로
   흩어졌고, 씬 수준 링크 7개 중 6~7개를 못 이었다. 씬 명세가 묻는 것을 안 본 것이다.

### 지어냄 수치를 액면가로 읽으면 안 된다

떨어진 인용 29건을 손으로 갈랐다.

- **11건(전부 arm b)은 하네스의 한계다.** 읽기를 `condition` 에서만 찾는데, 그 11건은 같은
  메서드의 `calls` 인자로 렌더에 **보인다** — `MapMove.Update` 가 `ShowBattle(MapMove.StagePosition)`
  를 부르는 것 같은 것. 모델이 지어낸 것이 아니라 스키마가 그 자리를 조건으로 안 적었을 뿐이다.
- **arm (c) 의 10건 중 6건은 프롬프트 결함이다.** (b) 에는 `unit` 을 `네임스페이스.타입.메서드`
  로 쓰라고 했지만 (c) 에는 모양을 안 알려 줬고, (c) 는 메서드를 뺀 타입 이름(`Tutorial.TutorialController`)
  을 냈다. **arm (c) 의 45.8% 는 형식 미준수이지 사실 창작이 아니다.** 다음 실행 전에 고쳐야 한다.
- 남는 것 중 깨끗한 사실 하나: **arm (a) 는 3 run 동안 대지 못할 인용을 한 건도 내지 않았다.**

### 권고 — TC agent 입력은 content_map 으로 간다. 단, 적재기를 고친다

- **의사 C# 을 주지 않는다.** 3배 비싸고(run 당 $0.44 대 $0.13), 2배 느리고(216s 대 116s),
  씬 수준 링크를 거의 다 놓친다(6.5/7). 게다가 TC 가 실제로 필요한 `status`·`selector`·
  `verification` 이 없다. `// reached from:` 이 조인 결과를 문장으로 들고 있다는 이점은
  확인됐지만, 그 이점은 arm (c)(둘 다)에서도 재현되지 않았다(맵 밖 골든 1/2).
- **둘 다 주는 것도 아니다.** 정답률이 (a) 보다 0.05 높은데 이는 run 두 개짜리 표본의
  노이즈 안이고, 비용과 시간은 (b) 와 같다.
- **이 실험의 진짜 발견은 입력 형식이 아니라 적재기다.** SC-6·SC-7 은 근거 캡처에 멀쩡히
  있는데 content_map 이 싣지 않은 사실이다 — `GameClearController.ShowGettedCard` 가
  `(MapMove.StagePosition - 1) == StageDataSingleton.stagePosition` 을 읽는 것,
  `GameClearController.Start` 가 `stagePosition == 4` 로 `EndingScene` 을 여는 것.
  **고칠 자리는 프롬프트 입력이 아니라 ARTEL-444 의 적재기다.** 적재기가 이 읽기들을 실으면
  arm (a) 의 상한이 그만큼 올라가고, 의사 C# 을 붙일 이유가 사라진다.

### 믿으면 안 되는 것

- 표본이 작다. arm 당 2~3 run, 게임 하나, 골든 10개. **골든 한 개 차이는 노이즈 안이다.**
  구조적인 차이는 하나뿐이다 — 맵 밖 도달(0/2 대 2/2). 그것은 표집이 아니라 설계다.
- 온도 0.2 이고 seed 를 넣을 자리가 없어 결정론이 아니다. arm (a) 는 같은 입력에 6·8·7 을 냈다.
- **기계가 못 잡는 실패는 그대로 남아 있다**: 양쪽 행에 같은 `via` 가 진짜 있는데 인과가 없는
  조합. 이 하네스는 그것을 통과시킨다. QA 런이 최종 판정자라는 이슈의 말은 유효하다.
- 모델 하나만 쟀다. 다른 모델에서 함정 셋이 나오지 않는다는 근거는 없다.

### 남은 일 (이 이슈 밖)

- 읽기를 `condition` 뿐 아니라 `calls` 인자에서도 인정할지 정한다. 정하면 지어냄 수치가
  arm (b) 에서 크게 내려간다. 근거 스키마의 계약 문제라 여기서 혼자 정하지 않았다.
- arm (c) 프롬프트에 `unit` 모양을 적고 다시 잰다. 재채점은 `--replay` 로 공짜다.
