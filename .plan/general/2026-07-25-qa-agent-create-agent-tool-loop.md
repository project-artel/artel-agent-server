...
## Decisions (2026-07-25 확정)

**스텝 판정 — 툴 + 최종 구조화 둘 다.** `report_step` 툴로 스텝마다 즉시 타임라인에
흘리고, 끝날 때 `response_format`으로 전체 요약을 받는다. 툴만 쓰면 모델이 호출을
빠뜨렸을 때 판정이 빈 실행이 나오고, 구조화 출력만 쓰면 실행 중에는 화면에 아무것도
보이지 않아 실시간 타임라인을 만든 의미가 없다. 둘을 겹쳐 서로의 실패를 막는다.

**상한 — 호출 수와 시간 둘 다.** 툴 호출은 스텝 수 비례로 `10 + 15 × 스텝수`,
전체 실행은 10분. 먼저 걸리는 쪽에서 FAILED로 종단하고 이유를 남긴다. 호출 수만 두면
응답 없는 게임에서 한 호출이 오래 매달려 시간이 무한정 늘고, 시간만 두면 짧은 시간에
호출을 폭발시키는 경우를 못 막는다.

**씬 전환 — 이름 변화만 초기화.** interactables 구성 변화까지 전환으로 보면, 화면이
바뀌는 바로 그 순간에 이전 값이 사라진다. "누르면 상점 화면이 뜬다" 같은 스텝은 정확히
그 전후 비교를 근거로 삼으므로, 그때 초기화하면 판정 근거를 스스로 지우는 셈이다.
대신 잔상이 쌓이지 않도록 `missing` 키에 수명을 둔다(N회 관찰 뒤 폐기).

**무응답 — 툴이 값으로 알린다.** 타임아웃은 예외로 루프를 끊지 않고 "응답 없음"을
반환한다. 재시도할지, 더 기다릴지, 실패로 판정할지는 에이전트가 정한다. 로딩이 길어
잠시 응답이 없는 것과 게임이 죽은 것을 서버가 구분할 수 없기 때문이다. 전체 시간
상한이 최종 안전장치다.
