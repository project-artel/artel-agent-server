...
**플래그는 카탈로그에서 확인한다.** 이슈 본문이 `google/gemma-4-31b-it:free`를 "못 보는
모델"로 지목해 그대로 따랐으나, OpenRouter 카탈로그의 `architecture.input_modalities`는
`image,text,video`다(툴 호출도 지원). 잘못된 플래그가 그 모델에서 캡처 툴을 조용히
빼앗고 있었다. 정정했고, `supports_strict_json`과 같은 방식으로 카탈로그에서 확인하도록
독스트링에 명시했다.

그 결과 카탈로그의 7개 모델이 전부 이미지를 본다. 텍스트 전용 경로를 실모델로 밟을 수단은
없어졌고, 유닛 테스트(`build_tools(..., supports_vision=False)`)가 그 경로를 덮는다.
실모델 검증이 필요해지면 툴은 되고 이미지는 안 되는 무료 모델이 카탈로그에 10개 있다
(`inclusionai/ling-3.0-flash:free` 등).
