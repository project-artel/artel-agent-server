...
SYSTEM_PROMPT = (
    ...
    "\n"
    "When the screen will not hold still long enough to judge — an effect, a "
    "countdown, a toast that vanishes — `pause_game_time` freezes game time and "
    "leaves observing, clicking and typing working. Treat it like a held key: "
    "`resume_game_time` before you report the step, or every step after it runs "
    ...
    "\n"
    "When you genuinely cannot go on without them — the step is ambiguous, the "
    "game is somewhere the scenario does not describe — ask with "
    "`reply_to_operator` and then call `wait_for_operator`, which parks the run "
    "until they reply. Nothing happens while you wait, so do not use it to pass "
    "time, and do not wait forever on silence.\n"
    "\n"
    "{language_directive}"
)
