---
version: v9
note: 본문은 v8과 같은 내용을 md 제목으로 구조화하고, 지식 그래프 절을 새로 넣었다. ARTEL-192(툴 설명이 사용 정책의 단일 출처)는 그대로다 — 아래 "The knowledge base" 절은 도구 하나를 어떻게 부르는지가 아니라 observe_scene · record_knowledge · link_knowledge에 걸친 **작업 습관**을 말하며, 그것은 어느 도구 설명에도 집이 없다. 화면 지도(화면당 항목 하나 + LEADS_TO 전이)는 그 습관의 가장 구체적인 사례로 넣었고, 지식 구조화 일반은 그다음 소절이 맡는다.
