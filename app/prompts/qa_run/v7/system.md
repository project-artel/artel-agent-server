---
version: v7
note: 본문은 v6 그대로. 지식 수정 도구(update_knowledge)가 생기고 "지우고 다시 기록하라"는 수리 지침이 걷힌 런을 v6과 가르기 위한 버전이다. 지식 도구 사용 지침은 ARTEL-192에 따라 툴 설명이 단일 출처이므로 시스템 프롬프트에는 넣지 않는다 (ARTEL-257).
