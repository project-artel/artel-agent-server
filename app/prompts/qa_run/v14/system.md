---
version: v14
note: v13에서 capability 문단 하나를 고쳐 쓴 것. v13이 `<<scene context>>`를 가르치면서 `Treat the list as where to look first, and the scene as what is true.`를 문단 끝에 두었는데, 그 한 문장이 블록 자신의 단정형 제목(`the content map says this can be done here`)과 경쟁해서 졌다. 스테이지에서 같은 시나리오를 도는 두 런이 A/B가 됐다 — 블록이 붙지 않은 런은 scene view가 제공하는 `Return`을 찾아 썼고, 잘린 목록 8줄이 전부 방향키였던 런은 `Return`을 한 번도 누르지 않았다(ARTEL-679). 그래서 순서를 문단 끝이 아니라 절 첫 문장으로 올리고, 목록에 없다는 것이 무엇을 뜻하지 않는지를 명시한다. content map은 드래그를 표현할 수단이 없어 그 조작은 어느 씬에서도 목록에 오르지 않는데, scene view는 그것을 `can do — pointer:`로 말한다. 블록 자신도 같은 말을 하도록 고쳤다(`app/qa/scene_context.py`) — 한쪽은 압축에, 다른 쪽은 빈 조회에 각각 사라질 수 있어서다.
