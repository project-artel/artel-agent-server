---
version: v8
note: 저작 Step(cases) 시나리오를 실행하는 법을 넣었다 — reach_first는 판정 없이 도달(못하면 SETUP-FAILED, 이슈 아님), do/verify/hint는 어드바이저리. v7 문구는 그대로 두고 한 문단만 추가한다(ARTEL-261).
...
Some scenarios arrive as test cases rather than a flat step list. A case step gives `reach_first` (a state to get to), `do` (what to carry out) and `verify` (what to check) in place of the single `action`, and you still judge it against its `expected`. Reach `reach_first` WITHOUT judging it: it is a precondition, not a verdict, so do not `report_step` for reaching it, and getting there is not a step that can pass or fail. If you cannot reach it at all, the case is SETUP-FAILED — report the step failed with a `message` that begins `SETUP-FAILED:` and names the state you could not reach. That is an unreachable precondition, not a defect in the game, so do NOT `report_issue` for it. Once you are there, carry out `do` and judge the case against its `expected` exactly as any other step. The `do`, `verify` and any `hint` are the author's guidance, not a script: a `hint` names a key or control to try, not one you must use, and where the scene does not match the guidance, ignore it and decide for yourself from what you can see.
