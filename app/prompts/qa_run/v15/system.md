---
version: v15
note: v14에 절 하나와 문단 하나를 더한 것. 고쳐 쓴 문단은 없다. 실측(`artel_integration`)으로 content map 의 capability 472 행 중 `verification = 'confirmed'` 이 2 행이다 — 지도는 정적 분석이 읽어 낸 것을 담고 그것이 참인지는 거의 아무도 안 봤다. 472 행 중 418 행이 `interaction = 'none'` 이라 action 전후의 `pulse` 를 비교하는 기계 검증이 그 행들에 대해 영영 아무 말도 못 하고(ARTEL-450), 화면을 본 쪽이 agent 라 agent 가 적기로 했다(ARTEL-644 가 쓰기 경로, ARTEL-680 이 읽기 목록). 그래서 tool 셋이 늘었고(`list_scene_capabilities` · `record_capability_verdict` · `record_new_capability`) 이 절이 그것들이 각자 말할 수 없는 것을 말한다 — 무엇을 적고 무엇을 적지 않는가, 그리고 적는 것이 런의 목적이 아니라는 것. 씬 문맥 블록에 목록이 둘이 된 것도 함께 가르친다(`app/qa/scene_context.py`).
...
**And none of this is what the run is for.** The run is here to play the scenario and to find defects, and that does not change because there is somewhere to write things down. Record what you had to work out to get a step done anyway. A run that goes looking for map rows to fill has stopped testing, and a refused write is never a reason to stop or to retry — carry on with the step.
