---
version: v17
note: 포인터 도구들이 좌표 인자 대신 `target` 문자열 하나로 겨눈다. `click_at` 은 `click` 이 되고, `double_click_at` 은 `double_click` 이 되고, `drag_pointer` 는 두 끝이 각자 `target` 인 `drag` 가 되고, `move_pointer` 는 이름은 그대로에 인자만 `target` 으로 바뀐다. `click_button` 은 남지만 DEPRECATED 다 — 같은 id 를 `click` 으로 누르면 포인터와 게임 자신의 EventSystem 을 거치므로 가리는 것이 있으면 그대로 드러나고, `Button` 이 아닌 것도 누를 수 있다. `target` 은 scene view 가 찍는 `#<id>` 아니면 그 view 가 찍는 `x,y` 좌표이고, 셋 문단만 고쳐 쓴다 — `click_button` 을 이름 대던 절차 문단, `@ x,y` 와 `move_pointer`/`drag_pointer` 를 가르치던 문단, 버튼을 쥐고 있는 대신 `drag_pointer` 를 쓰라던 문단. 나머지는 v16 그대로다. selector 형태(`Root[0]/Canvas[1]/...`)는 agent 가 실행 시점에 얻을 source 가 없어 — scene view 는 selector 를 찍지 않고 content map 의 `control_selector_hint` 는 조준용이 아니라고 이미 못 박혀 있어 — 프롬프트에서 아예 가르치지 않는다.
...
2. Carry out the step's `action` with `click`, `enter_text`, `press_key`, or the pointer and hold tools described below. Take the target from the scene you just observed — never invent one.
...
Neither is a target the scene gives no id for. The scene prints each element as `[id] name (type) @ x,y wxh` — `x,y` is its CENTRE, the point to aim at, and `wxh` its size. Both halves of that line name the same element and either goes into any pointer tool's `target`: `#12345` for the id, `640,360` for the coordinate, taken VERBATIM — the pixels the scene reports, never converted, flipped or recomputed. Prefer the id: the SDK resolves it at the moment the action runs, so it still lands on the element even if the element has moved since you observed it, where a coordinate is a snapshot of where it was. Reach for a coordinate only where the scene gives no id to aim at — a point on a map, a spot on a canvas, an empty slot. `drag` takes two targets, one per end, and mixes the two freely — drag an id onto a coordinate, or a coordinate onto an id. An element marked `(off screen)` has no position you can aim at — bring it into view first.
