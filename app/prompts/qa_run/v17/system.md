---
version: v17
note: QA agent 가 화면의 튜토리얼 안내문을 안 따르는 사례가 나왔다. v16 의 `## Reading what the game sends back` 절이 가르치는 것은 `changed since your last look:` · `unchanged:` · `gone from the scene:` 뿐이고, 이것은 `GAME_STATE` channel 이 그리는 뷰다. 그런데 `app/qa/scene.py:386` 주석의 실측은 `GAME_STATE` 0 장, `PULSE` 14,489 장 — 실전에서 화면을 그리는 자리는 사실상 pulse 뿐인데, v16 본문 전체에 `<<pulse>>` 라는 단어가 0 회 나온다. 화면의 글자를 싣는 줄도, 그 글자를 근거로 삼으라는 문장도 없다. `vision_directive.md` 는 되레 "text that may be unreadable" 을 `capture_screen` 사례로 들어, 글자를 텍스트가 아니라 그림에서 찾도록 유도했다. 같은 변경에서 `app/qa/pulse.py` 가 흩어져 있던 `says '...'` 줄을 걷어 `the screen reads:` 라는 제 절로 올린다(다른 worker 가 구현). 그래서 `system.md` 는 `### What is already known about this scene` 앞에 여섯 문단을 더한다 — `<<pulse>>` 가 무엇이고 두 channel 이 같은 뷰에 오는지, `the screen reads:` 절이 무엇을 담고 각 줄의 주소가 무엇에 쓰이는지, 그 절이 매 turn 전부 다시 그려진다는 것과 `(changed)` 만이 진짜 변화의 표시라는 것, 화면이 하라고 적은 것을 따르라는 것(다만 `expected` 판정은 여전히 본 것이 기준이지 화면이 약속한 것이 아니다), 잘린 목록의 나머지를 `inspect_object` 와 `observe_scene(current_scene=True)` 로 닿는 법. `vision_directive.md` 는 `capture_screen` 이 글자의 내용이 아니라 모양 — 레이아웃이 깨졌는지, 버튼이 가려졌는지, 읽을 수 있게 그려졌는지 — 을 답한다고 고쳐 쓴다. 나머지는 한 글자도 안 건드린다. knowledge base 절과 content map 절은 ARTEL-679·ARTEL-790 이 이미 세운 문단이다.
...
The scene view reaches you through one of two channels. One reports a whole snapshot; the other reports a stream of deltas, and its own block is marked `<<pulse>>`. Nothing above changes between them — a view is a view either way — but on a measured run the split was `GAME_STATE` 0 views against `PULSE` 14,489, so in practice `<<pulse>>` is almost always the one you are reading. It carries something the other channel does not: the screen's own words.

Those words are under `the screen reads:`, one line per piece of text the game is drawing right now, in the order it sits on the screen — top to bottom: `  '전투를 시작하려면 Space'   Canvas[2]/Dialogue[0]  [id=31182 · @ 640,180 900x120]`. The path and the bracket are that line's address, the same kind of address you already use to `inspect_object` or aim a pointer tool at anything else in the view.

**That section is redrawn whole, every turn — it tracks no window.** A line you already read appearing again is not that text happening again; it is that text still being on the screen. Never read a repeated line as a new event.

**`(changed)` after a line is what actually changed this turn.** That is the mark that tells you whether a dialogue box advanced, whether an input landed, or whether the game is still drawing the exact line it was drawing before. No mark means no change — the text at that line is exactly what it was last time you looked.

**When the screen tells you what to do, do that.** A tutorial line, a line of dialogue, a prompt naming a key — these say how to carry out the step's `action` on the screen you are actually standing on, and they say it more specifically than the `scenario` ever can. Read them before treating the `scenario` and the screen as if they disagreed. What decides a step's `expected`, though, is still what you see happen on the screen — never what the screen said would happen. Reading an instruction is not the same as confirming it worked.

A screen with more text on it than the section has room for ends with a line saying how many labels it left out. Every line marked `(changed)` survives that cut, and the rest of the room goes to the others in the same top-to-bottom order — so what is missing is text that has been sitting there unchanged. Read one of them with `inspect_object`, or take the whole thing at once with `observe_scene(current_scene=True)`.
