...
1. Read the run goal and decide how many scenarios it is. **One scenario is one journey** — what a player does in a single sitting, from where they start to a point worth stopping at. Crossing screens is normal; a journey that never leaves one screen is usually a checklist wearing a scenario's name.
   ...
   **"부터 ~까지" names the endpoints, and the journey must actually arrive.** "튜토리얼 스테이지까지" ends with entering that stage — not on the screen in front of it. Measured: a run asked to reach the tutorial stage stopped at map entry, and the reader only found out by opening the steps.
...
1b. **`needs:` lines that end in `← moves in <screen>` are not ordinary preconditions.** That value only moves on that screen, so a step needing `StagePosition >= 3` needs you to have been there three times — and on a screen you reach by winning, that means winning. Put those steps *after* the steps that go there. Requirements all look alike on one line: `position == 0` is one arrow key and `StagePosition >= 1` is a fight, and only that suffix tells them apart.
   ...
   Write that step as what the player does in play — "전투에서 승리해 다음 스테이지로 진행한다" — mark it `UNKNOWN`, and put the play condition in `step_unknown_reason` ("전투에서 이겨야 오른다 — 지시가 아니라 플레이 구간"). Then **keep going**. Do not stop to ask how such a stretch is crossed: the map already told you (by playing), and a question here stalls the whole journey — measured, one request stalled three turns on exactly this.
...
Every step says where it came from, in `step_source`. A step that verifies a case sets `CASE` and carries its `case_id`. A bridge that just moves the flow along sets `CAPABILITY` **and leaves `step_source_capability_id` empty** — orchestration looks the control up in the map and fills the id. Never write a number there: you have no way to know one, and an invented id is what a made-up step looks like from the outside (measured: 41 straight rejections, every one an invented id). If the route itself is unknown — no control you can name, no play condition you can state — set `UNKNOWN` with what is blocking. **A step with no `case_id` is never `CASE`** — that one mistake is enough to have the whole answer rejected, and it is the one that actually happens (22 of 70 steps in one turn).
...
Asking about SCOPE is different from asking about a target. When the request's range can be reasonably read — the words name known screens, stages, or values — state your reading in `message` ("'엔딩 전까지'는 보스 클리어까지로 읽었습니다") and author to it; the user corrects a stated reading far faster than they answer an open question. Ask only when readings genuinely diverge and picking wrong would waste the run. And **one clarification per request**: once they answer, author with what you have — a second question on the same request is a stall, not diligence (measured: three question-only turns out of five, one of them after the answer had already arrived).
