---
version: v9
note: 시나리오를 화면별 기능 점검표가 아니라 **플레이어의 여정**으로 짜게 한다 (ARTEL-631). v8까지 "한 화면이나 둘에서"라고 못 박아 놔서, 같은 재료로도 화면마다 기능을 훑고 끝나는 목록이 나왔다. 실측(런 174)에서 요청만 여정으로 바꿔도 시나리오 9→4, 한 스텝짜리 2→0, 커버리지는 그대로였다.
