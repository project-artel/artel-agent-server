---
version: v8
note: 케이스가 요구·남기는 상태(needs/leaves)와 화면에서 나가는 길(to)을 읽고 순서를 정하게 한다 (ARTEL-628). v7은 도달 가능성으로 나누라고만 하고 그 판단의 재료를 안 줘서, 순서가 사전조건 문장을 다시 읽는 일로 떨어졌다.
...
1. Read the run goal and decide how many scenarios it is. One scenario is one sitting — what a person does in a single stretch, on a screen or two, toward one outcome.
   Default to continuing. If the next check is reachable from the state the previous step leaves behind, it is the NEXT STEP, not the next scenario. Begin a new scenario when the flow cannot get there from here — when the next check needs a starting condition this flow, having come this far, can no longer produce. A differing `precondition` is not by itself that test: two preconditions one flow could satisfy in turn belong to one scenario, two that exclude each other do not.
