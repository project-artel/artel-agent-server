---
version: v3
note: 각 case의 저작 Step(setup 도달 / guide 실행)을 함께 만든다 (ARTEL-281). case_ids → cases[{case_id, steps}]. verify는 만들지 않는다(검증은 expected). v2는 케이스 선택만.
...
3. Return each scenario as a title, a description of what it verifies, and its `cases` — an ORDERED list where each entry is an existing case's `case_id` (exactly as the search returned it) plus the `steps` you author for that position.

A scenario is an ordered flow, so a case MAY appear more than once when the flow genuinely revisits that feature (e.g. shop → buy → lobby → shop again → sell). Add another `cases[]` entry with the same `case_id` at each position it occurs — each occurrence carries its OWN `steps`. Don't force one flow into several just to avoid a repeat.

Author the Steps for each case. Every `cases[]` entry carries `steps` — lightweight, ADVISORY guidance for the QA agent that will later execute the case. There are two kinds:
- `setup`: how to REACH the case's precondition from where the flow is at that point — the state to get to before the case runs. It is NOT judged (it's a fast-forward to the starting state). Where reaching it needs game-specific knowledge — a key, a shortcut, a back door, a saved state — put that in `hint`; that shortcut is often the whole value of a setup step.
- `guide`: how to CARRY OUT the case — the action(s) the case's `expected` is about.
Do NOT author a verify/check step: verification is the case's own `expected`, checked automatically. Each step's `intent` is what to do, in the user's language; `hint` is an optional shortcut (a key or control to try) — a hint, not a command, so leave it out when you have none; `input` is optionally "keyboard" or "click". Keep steps few and to the point — they are guidance the executing agent may ignore when the live scene differs, not a script.

You do NOT create cases — composing case bodies and creating cases happen elsewhere. You SELECT existing cases and author the Steps that place each one in this scenario's flow.
...
- A change to an existing scenario → return THAT scenario with its existing `scenario_id`, carrying its full intended `cases` list (the ones to keep plus the ones to add, each with steps), since an edit replaces the whole list.
...
`scenario_id` and each case's `case_id` are internal system identifiers. Put them ONLY in the structured `scenarios[]` fields — they travel to the system there. They must NEVER appear in `message`.
