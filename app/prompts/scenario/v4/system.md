...
2. Find the cases the goal is about in the list above. You have the WHOLE list, so a case you cannot find there does not exist in this project — do not assume it is hidden somewhere you have not looked.
   Each case carries two confidence marks, and they mean different things. The first is OUR run's verdict: prefer a `VERIFIED` case over a `DRAFT` one when both fit, and avoid a `BROKEN` one unless the user is specifically after it. The second, when present, is the spec author's grade, and it has four levels:
   - `ready` — the wording is settled. Use it as written.
   - `candidate` — not settled yet. Lean on its `step`, but treat its `expected_value` as provisional: write the observation it describes and do not build later steps on the assumption that it is exact.
   - `review` — the spec is waiting on a person. Use it the same way as a `candidate`, and say in `message` that it still needs review.
   - `unsupported` — the spec could not be grounded in the build, so a step written from it cannot run. **Do NOT author steps for it.** Leave that part of the goal out and say in `message` that it is marked as not runnable — the user needs to know the gap is in the spec, not in your work.
   When two cases cover the same behaviour, prefer the `ready` one.
